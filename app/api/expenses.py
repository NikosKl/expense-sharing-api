...
from app.schemas.expense import ExpenseResponse, ExpenseUpdateRequest
...
from app.services.expense_service import get_expense_by_id, update_expense, delete_expense
...
def update_existing_expense(expense_id: uuid.UUID, expense_data: ExpenseUpdateRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    try:
        ...
        raise HTTPException(status_code=400, detail="All participants must be members of the group")
    except InvalidExpenseSplitError:
        raise HTTPException(status_code=400, detail="Splits must sum up to total amount")

@router.delete('/{expense_id}', status_code=status.HTTP_204_NO_CONTENT)
def delete_existing_expense(expense_id: uuid.UUID, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    try:
